...
bool hasFilledSelfAppraisal(const std::string& username) {
  ...
  if (pos != std::string::npos) {
    ...
    std::ifstream file("data/self_appraisal.txt");
    std::string line;
    while (getline(file, line)) {
...
void updateSelfAppraisal(const std::string& username, int task1Rating, int task2Rating, int task3Rating, int task4Rating) {
  ...
  std::ofstream file("data/self_appraisal.txt", std::ios_base::app);
...
bool hasBossRatedEmployee(const std::string& employeeName) {
    std::ifstream file("data/boss_ratings.txt");
    std::string line;
    while (getline(file, line)) {
    ...
    if (checkCredentials(bossCredentials, username, password)) {
        ...
        if (it != selectedEmployees.end()) {
            ...
            } else 
            {
                ...
                tasker obj2;
                task(obj2);
                std::ofstream file("data/boss_ratings.txt", std::ios_base::app);
...
void loginAsHR(const std::unordered_map<std::string, std::string>& hrCredentials) {
    ...
    if (checkCredentials(hrCredentials, username, password)) {
        ...
        if (it != hrEmployees.end()) {
            ...
            std::ifstream selfAppraisalFile("data/self_appraisal.txt");
            std::ifstream bossRatingsFile("data/boss_ratings.txt");
...
int main() {
    std::unordered_map<std::string, std::string> employeeCredentials = readCredentialsFromFile("data/employee_credentials.txt");
    std::unordered_map<std::string, std::string> bossCredentials = readCredentialsFromFile("data/boss_credentials.txt");
    std::unordered_map<std::string, std::string> hrCredentials = readCredentialsFromFile("data/hr_credentials.txt");
