...
Appraisal::Appraisal(double value) : value(value) {}

Appraisal::operator double() const {
    return value;
}

bool Appraisal::operator>=(const Appraisal& other) const {
...
void calculations::checkYearlyAppraisal(int employeeID) {
    std::ifstream file("data/final.txt");
...
void calculations::calculateAppraisal(std::unordered_map<int, std::string>& hrEmployees) 
{
    std::ifstream bossRatingsFile("data/boss_ratings.txt");
    std::ofstream finalFile("data/final.txt");
